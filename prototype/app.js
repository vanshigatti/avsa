...
function renderViralitySignals(text) {
  ...
  if (!list) return;
  list.innerHTML = "";
  ["breaking", "urgent", "share", "forward", "alert", "emergency"].forEach(
    (t) => {
...
function ratingClass(r) {
  ...
  if (
    /false|fake|hoax|incorrect|inaccurate|misrepresent|pants.on.fire|pants.fire|four.pinocchio|three.pinocchio|two.pinocchio|fabricated|scam|debunked|mislead|distorted|unsupported|lacks.context|missing.context|no.evidence|baseless|unfounded|wrong|disputed/.test(
      t,
    )
  )
    return "false";
  if (
    /mixture|mixed|partially|half|mostly false|some truth|partly|unverified|needs.context|unproven|inconclusive|flawed/.test(
      t,
    )
  )
    return "mixed";
  if (
    /true|correct|accurate|verified|confirmed|mostly true|largely true|supported/.test(
      t,
    )
  )
    return "true";
...
function renderEvidence(reviews) {
  ...
  reviews.slice(0, 6).forEach((r) => {
    const li = document.createElement("li");
    const rc =
      ratingClass(r.rating) === "false"
        ? "#e84c7f"
        : ratingClass(r.rating) === "true"
          ? "#2f8f6b"
          : "#f7a07e";
    li.innerHTML = `
...
async function showPropagation(
  newsText,
  fakeProbability,
  verdict,
  factCheckReviews,
...
async function analyzeClaim() {
  ...
  try {
    ...
    finalScore =
      typeof groq.score === "number" ? calibrateScore(groq.score) : heuristic;

    const lowerLabel = (groq.label || "").toLowerCase();
    if (lowerLabel.includes("false") || lowerLabel.includes("fake"))
      finalVerdict = "fake";
    else if (lowerLabel.includes("true") || lowerLabel.includes("real"))
      finalVerdict = "real";
